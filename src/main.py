...
def main(
    ...
    if not args.data_aug:
        ...
        eval_transformation_list.extend(
            [transforms.Resize((32, 32)), transforms.Normalize(mean=[.5], std=[.5])])
    ...
    out_filename = 'BSz_' + str(batch_size) + '_LR_' + str(lr) + '_Optim_' + str(optimizer_fn) + '_LossFn_' + str(
        loss_fn) + '_Margin_' + str(margin) + '_ED_' + str(epoch_decay) + '_WD_' + str(weight_decay) + args.comment
    if args.data_aug:
    ...
    for epoch in trange(num_epochs):
        ...
        if cur_auc > best_auc:
            ...
            model_save_filename = os.path.join(
                output_root, out_filename, 'best_model.pth')
